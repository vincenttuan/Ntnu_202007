...
def open_img(x):  # x -> 檔名
    ...
    img = ImageTk.PhotoImage(img)  # 當檔案轉成影像物件
    panel = tk.Label(app, image=img)  # 將影像物件放入到 Label
    panel.image = img  # 將影像物件放入到 Label
...
def submit():
    ...
    labelValue.set("數量：{0}\n品名：{1}\n單價：{2}\n糖分：{3}\n提袋：{4}\n小計：{5}"
                   .format(nums.get(), comboExample.get(), drinks[comboExample.get()], sugar[radioValue.get()], bag(chkValue.get()), nums.get() * drinks[comboExample.get()] + extra(chkValue.get())))
